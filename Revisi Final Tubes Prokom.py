...
import pandas as pd
from tkinter import *
from tkinter import messagebox, Canvas, StringVar, Entry, Button
from datetime import datetime
from tkinter import ttk, Frame, Tk, OptionMenu, Listbox, Label
...
def tambah_barang():
    ...
    try:
        ...
        resized_image = image.resize((1560, 800))  
        bg_image = ImageTk.PhotoImage(resized_image)  
        canvas.create_image(0, 0, image=bg_image, anchor="nw")
        canvas.image = bg_image  
    except Exception as e:
...
def daftar_barang():
    def edit_harga():
        ...
        if not selected_item:
            messagebox.showerror("Error", "Pilih barang yang ingin diubah harganya!")
            return
        nama = tree.item(selected_item, "values")[0]
        harga_baru = harga_var.get()
        if not harga_baru.isdigit():
            messagebox.showerror("Error", "Masukkan harga yang valid!")
            return
        harga_baru = int(harga_baru)
        # Perbarui harga di DataFrame dan file Excel
        data.loc[data['Nama Barang'] == nama, 'Harga'] = harga_baru
        update_excel()
        messagebox.showinfo("Sukses", f"Harga barang '{nama}' berhasil diubah menjadi Rp{harga_baru:,.0f}!")
        harga_var.set("")
    ...
    def hapus_barang():
        ...
        if not selected_item:
            ...
            return
        nama = tree.item(selected_item, "values")[0]
        if messagebox.askyesno("Konfirmasi", f"Apakah Anda yakin ingin menghapus barang '{nama}'?"):
            # Hapus data dari DataFrame dan file Excel
            global data
    ...
    def update_tree():
        # Refresh data dari database (file Excel)
        global data
        data = pd.read_excel(file_path)
    ...
    # Judul
    canvas.create_text(770, 80, text="Daftar Barang", font=("Bubble Bobble", 50, "bold"), fill="black")

    # Treeview untuk tabel
    columns = ("Nama Barang", "Harga", "Stok")
    tree = ttk.Treeview(daftar_menu, columns=columns, show="headings", height=20)
    tree.place(relx=0.5, rely=0.43, anchor="center")
    ...
    # Tambahkan data awal ke Treeview
    update_tree()

    # Input harga baru untuk edit harga
    harga_var = StringVar()
    Entry(daftar_menu, textvariable=harga_var, font=("Arial", 10)).place(relx=0.5, rely=0.73, anchor="center")

    # Tombol
    Button(daftar_menu, text="Kembali", command=lambda: show_frame(main_menu), bg="#CC0000", fg="white", font=("Arial", 11)).place(relx=0.5, rely=0.87, anchor="center")
    Button(daftar_menu, text="Edit Harga", command=edit_harga, bg="#4CAF50", fg="white", font=("Arial", 11)).place(relx=0.5, rely=0.77, anchor="center")
    Button(daftar_menu, text="Hapus Barang", command=hapus_barang, bg="#f44336", fg="white", font=("Arial", 11)).place(relx=0.5, rely=0.82, anchor="center")
...
def transaksi_penjualan():
    ...
    def reset_form():
        ...
        if not selected_item:
            messagebox.showerror("Error", "Pilih barang yang ingin dihapus dari daftar transaksi!")
            return
